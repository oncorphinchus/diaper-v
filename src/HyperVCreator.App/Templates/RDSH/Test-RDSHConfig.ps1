# Test-RDSHConfig.ps1
# This script tests the Remote Desktop Session Host configuration script and validates its functionality
...
$TestVMName = "Test-RDSH01"
$TestCollectionName = "Test Collection"
$TestConnectionBroker = "rdcb.test.local"
$TestLicenseServer = "rdls.test.local"
...
function Test-ScriptSyntax {
    ...
    $ScriptPath = Join-Path $PSScriptRoot "Deploy-RDSH.ps1"
...
function Test-ParameterValidation {
    ...
    try {
        ...
        $RequiredParams = @("VMName", "ConnectionBroker", "CollectionName", "LicenseMode")
        ...
        $BoolParams = @("UseHA", "UseDHCP", "InstallOffice", "InstallRSAT", "EnableRemoteFX", "EnableUserProfileDisks")
        $StringParams = @("VMName", "ConnectionBroker", "CollectionName", "LicenseServer", "LicenseMode", 
                         "IPAddress", "SubnetMask", "DefaultGateway", "PreferredDNS", "UserProfileDiskPath")
        $IntParams = @("MaxConnections", "UserProfileDiskMaxSizeGB")
        ...
        foreach ($Param in $Parameters) {
            ...
            if ($IntParams -contains $ParamName) {
                $ParamType = $Param.StaticType.Name
                if ($ParamType -ne "int" -and $ParamType -ne "Int32") {
                    Write-Host "Parameter $ParamName should be integer, but is $ParamType" -ForegroundColor Yellow
                    $ParamTypeMismatch = $true
                    $TestResults.Warnings++
                }
            }
...
function Test-TemplateReplacement {
    ...
    try {
        ...
        $MissingPlaceholders = $Parameters | Where-Object {
            ...
            # Integer parameters may have a different format {{ParamName}} without quotes
            elseif ($_.StaticType.Name -eq "int" -or $_.StaticType.Name -eq "Int32") {
                -not ($ScriptContent -match "\{\{$ParamName\}\}")
            }
            else {
                -not ($ScriptContent -match "\{\{$ParamName\}\}")
...
function Test-ScriptLogic {
    ...
    try {
        ...
        $RequiredParts = @(
            ...
            @{ Name = "Role Installation"; Pattern = "Install.+RDS-RD-Server" },
            @{ Name = "RDS Configuration"; Pattern = "Configuring Remote Desktop Session Host" },
            @{ Name = "Licensing Configuration"; Pattern = "RDS licensing" },
            @{ Name = "RDS Deployment"; Pattern = "RDS deployment" },
            @{ Name = "User Profile Disks"; Pattern = "User Profile Disks" }
        )
